...
namespace Neo.SmartContract.Native
{
        ...
        where TokenState : NFTState, new()
    {
        ...
        protected async Task Mint(ApplicationEngine engine, TokenState token)
        {
            ...
            await PostTransfer(engine, null, token.Owner, token.Id);
        }

        protected Task Burn(ApplicationEngine engine, byte[] tokenId)
        {
            return Burn(engine, CreateStorageKey(Prefix_Token).Add(GetKey(tokenId)));
        }

        private protected async Task Burn(ApplicationEngine engine, StorageKey key)
        {
            ...
            engine.Snapshot.GetAndChange(CreateStorageKey(Prefix_TotalSupply)).Add(-1);
            await PostTransfer(engine, token.Owner, null, token.Id);
        }
